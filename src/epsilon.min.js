...
var Transform = exports.Transform = function () {
	function Transform(element) {
		...
		this.actions = defaults.actions.$clone(true);
		this.units = defaults.units.$clone();
	...
	_createClass(Transform, [{
		...
		value: function apply(data) {
			...
			if (actions.transition) this.element.css({ "transition": actions.transition + units.transition });

			if (actions.origin) this.element.css({ "transform-origin": actions.origin[0] + units.origin + " " + actions.origin[1] + units.origin });

			if (actions.backface == false) this.element.css({ "backface-visibility": "hidden" });

			if (actions.style) {
				if (actions.style == "3d") this.element.css({ "transform-style": "preserve-3d" });else if (actions.style == "flat") this.element.css({ "transform-style": "flat" });
			}

			this.element.css({ "transform": this._build(actions, units) });
		...
		value: function write(data) {
			...
			if (data.units) this.units.$join.left(data.units);

			if (data.transition && typeof data.transition == "number") actions.transition = data.transition;

			if (data.origin && data.origin.length == 2) actions.origin = data.origin;

			if (data.backface !== undefined) actions.backface = data.backface;

			if (data.style) actions.style = data.style;

			if (data.perspective && typeof data.perspective == "number") actions.perspective = data.perspective;
