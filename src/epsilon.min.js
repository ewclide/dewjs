...
exports.DocumentTools = undefined;
...
var DocumentTools = exports.DocumentTools = function () {
    function DocumentTools(elements) {
        _classCallCheck(this, DocumentTools);
    ...
    _createClass(DocumentTools, [{
        ...
        value: function _wrapAsync(element) {
            ...
            if (tag == "img" && element.complete) async.run.success();else if (tag == "img" || tag == "link" || tag == "script" || tag == "frame") element.addEventListener("load", function () {
                async.run.success();
        ...
        value: function before(doc) {
            var del = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
        ...
        key: '_insert',
        value: function _insert(doc, del, method) {
            var self = this,
                result = [];
        ...
        value: function wrap(classList) {
            if (typeof classList == "string") {
                ...
                var wrapper = DOC.create("div", classList[0]),
                    inside = "";
        ...
        key: '_buildTransform',
        value: function _buildTransform(actions) {
            var units = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
            ...
            for (var name in actions) {
                ...
                switch (name) {
                    case "translate":
                        if (action.length == 2) result += "translate(" + action[0] + unit + "," + action[1] + unit + ") ";else if (action.length == 3) result += "translate3d(" + action[0] + unit + "," + action[1] + unit + "," + action[2] + unit + ") ";
                        break;
                    case "rotate":
                        if (typeof action == "number") result += "rotate(" + action + unit + ") ";else if (Array.isArray(action)) {
                        ...
                        break;
                    case "scale":
                        if (typeof action == "number") result += "scale(" + action + ") ";else if (Array.isArray(action)) result += "scale(" + action[0] + "," + action[1] + ") ";
                        break;
                    case "skew":
                        if (typeof action == "number") result += "skew(" + action + unit + ") ";else if (Array.isArray(action)) result += "skew(" + action[0] + unit + "," + action[1] + unit + ") ";
                        break;
                    case "perspective":
        ...
        value: function convert(elements) {
            if (elements.nodeType == 1 || elements.nodeType == 9) return new DocumentTools(elements);else if (elements.isDocTool) return elements;else return false;
        }
    }, {
        key: 'merge',
        ...
        get: function get() {
            ...
            return {
                attach: function attach(list) {
                    ...
                    if (!DOC._eventList[self._id]) DOC._eventList[self._id] = {};
        ...
        get: function get() {
            ...
            return {
                ...
                get: function get(element) {
                    if (element) return jsonConverter.convert(element);else if (self.elements.length) {
                        var result;

                        if (self.elements.length == 1) result = jsonConverter.convert(self.elements[0]);else {
                            result = [];
                            self.elements.forEach(function (element) {
...
function clone(object, full) {
    function Clone(field) {
        for (field in object) {
            if (object.hasOwnProperty(field)) this[field] = full && _typeof(object[field]) == "object" ? clone(object[field], true) : object[field];
        }
    }
...
function removeIndex(array, index) {
	if (index < array.length - 1 && index > 0) return array.splice(index, 1);else return false;
}

function removeValue(array, value) {
	var index = array.indexOf(value);
	if (index != -1) return array.splice(index, 1);else return false;
}
		...
		return {
			index: function index(_index) {
				if (Array.isArray(_index)) {
					...
					return result;
				} else return removeIndex(self, _index);
			},
			value: function value(_value) {
				if (Array.isArray(_value)) {
					var result = [];

					result = _value.map(function (i) {
						return removeValue(self, i);
					});

					return result;
				} else return removeValue(self, _value);
			},
...
function istype(value, type) {
	if (Array.isArray(type)) {
		...
		if (typeof value == "number") return "number";else if (typeof value == "string") return "string";else if (typeof value == "boolean") return "boolean";else if (Array.isArray(value)) return "array";else if (typeof value == "function") return "function";else if (value.nodeType == 1) return "dom";else return "object";
	}
}
...
var URL = exports.URL = function () {
	...
	_createClass(URL, [{
		...
		value: function get(params) {
			...
			if (params === undefined) return all;else if (istype(params, "string")) return all[params];else if (istype(params, "array")) {
				var arr = {};
...
var _documentTools = __webpack_require__(1);

var proto = _documentTools.DocumentTools.prototype,
    DOC = new _documentTools.DocumentTools(document);
...
DOC.$define({
    ...
    _eventList: {},
    _runEvFunc: function _runEvFunc(id, type, e) {
    ...
    create: function create(tag, attr, css) {
        var doc = new _documentTools.DocumentTools(document.createElement(tag));

        if (typeof attr == "string") doc.addClass(attr);else if ((typeof attr === 'undefined' ? 'undefined' : _typeof(attr)) == "object") doc.attr.set(attr);
    ...
    _createClass(JsonConverter, [{
        ...
        value: function wrap(json) {
            if (!json._doc) {
                ...
                doc = new _documentTools.DocumentTools(element);
                ...
                for (var item in json) {
                    switch (item) {
                        case "text":
                            doc.text(json.text);break;
                        case "html":
                            doc.html(json.html);break;
                        case "value":
                            doc.value(json.value);break;
                        case "checked":
                            doc.checked(json.checked);break;
                        case "attrs":
                            doc.attr.set(json.attrs);break;
                        case "css":
                            doc.css(json.css);break;
                        case "transform":
                            doc.transform(json.transform, json.transform.units);break;
                        case "nodes":
                ...
                if (json.events) doc.event.attach(json.events);
        ...
        value: function _bind(json) {
            ...
            if (json.bind) {
                for (var item in json.bind) {
                    switch (item) {
                        case "text":
                            if (json.bind.text) bind.change(json, "text", function (value) {
                                json._doc.text(value);
                            });
                            break;
                        case "html":
                            if (json.bind.html) bind.change(json, "html", function (value) {
                        ...
                        case "value":
                            if (json.bind.value) {
                                ...
                                if (json.tag == "input" && json.attrs.type == "text" || json.tag == "textarea") json._doc.event.attach({
                        ...
                        case "checked":
                            if (json.bind.checked) {
                                ...
                                if (json.tag == "input" && (json.attrs.type == "checkbox" || json.attrs.type == "radio")) json._doc.event.attach({
        ...
        value: function convert(element) {
            if (element.nodeType == 1) {
                ...
                if (elements.length) {
                    ...
                    for (var i = 0; i < elements.length; i++) {
                        if (elements[i].nodeType == 1) result.nodes.push(this.convert(elements[i]));else if (elements[i].nodeType == 3) result.text = elements[i].textContent;
                    }
                }
