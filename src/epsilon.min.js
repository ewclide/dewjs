...
var DocumentTools = exports.DocumentTools = function () {
    ...
    _createClass(DocumentTools, [{
        ...
        key: '_buildTransform',
        value: function _buildTransform(actions, units) {
            var result = "";
        ...
        get: function get() {
            ...
            return {
                apply: function apply(actions) {
                    var selfActions = self._transformState.actions,
                        units = self._transformState.units;

                    for (var i in actions) {
                        if (actionsList.$have(i)) self._transformState.actions[i] = actions[i];
                    }

                    if (selfActions.transition) self.css({ "transition": selfActions.transition + units.transition });

                    if (selfActions.origin && selfActions.origin.length == 2) self.css({ "transform-origin": selfActions.origin[0] + units.origin + " " + selfActions.origin[1] + units.origin });
                ...
                units: function units(_units) {
                    ...
                    if ((typeof _units === 'undefined' ? 'undefined' : _typeof(_units)) == "object") for (var i in _units) {
                        if (i in state.units) state.units[i] = _units[i];
                    }
                },
                reset: function reset(units) {
...
var _doc = __webpack_require__(13);
...
window.$define({
	...
	DOC: _doc.DOC // ? need thing about symbol $ in begin
});
            ...
            if (!json._doc) {
                ...
                for (var item in json) {
                    switch (item) {
                        ...
                        case "transform":
                            if (json.transform.units) doc.transform.units(json.transform.units);
