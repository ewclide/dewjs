    ...
    _createClass(DocumentTools, [{
        ...
        value: function transform(data) {
            if (data.reset) {
                ...
                this._transformState.units = _transform.units;
            }
            ...
            for (var i in data) {
                if (_transform.actions.$have(i)) actions[i] = data[i];
            }

            if (actions.transition) this.css({ "transition": actions.transition + units.transition });

            if (actions.origin) this.css({ "transform-origin": actions.origin[0] + units.origin + " " + actions.origin[1] + units.origin });

            if (actions.backface !== undefined && actions.backface == false) this.css({ "backface-visibility": "hidden" });

            if (actions.style) {
                if (actions.style == "3d") this.css({ "transform-style": "preserve-3d" });else if (actions.style == "flat") this.css({ "transform-style": "flat" });
            }
        ...
        value: function _buildTransform(actions, units) {
            ...
            if (actions.perspective) result += "perspective(" + actions.perspective + units.perspective + ") ";
                ...
                switch (name) {
                    ...
                    case "rotate":
                        if (typeof action == "number") result += "rotate(" + action + unit + ") ";else if (Array.isArray(action)) {
                            ...
                            if (action[2] !== undefined) result += "rotateZ(" + action[2] + unit + ") ";
                        }
                        break;
                    case "scale":
...
Object.prototype.$define({
    $join: function $join(objects) {
        var have = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
DOC.ready(function () {
    DOC.body = new _documentTools.DocumentTools(document.body);
});
            ...
            if (!json._doc) {
                ...
                for (var item in json) {
                    switch (item) {
                        ...
                        case "transform":
                            doc.transform(json.transform);break;
                        case "nodes":
